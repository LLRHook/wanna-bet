...
The English caption appears with a small source-language label and the provider's native gallery preview in one message. Long captions continue in a text attachment. This preserves the native preview where it works; it does not build a custom media player. A failed single-post preview keeps the original message and a tracked English-caption output. A mixed/multiple-post preview failure uses the existing rollback path. No bot join message is added.
...
Those observations concern this post at that time. A successful metadata request or `GraphImage` hint does not establish working media. Automated tests use synthetic captions and media URLs, and cannot establish the Google translation's accuracy or Discord playback.
...
No live Google translation or Discord translation render has been verified for this feature yet. The Cloud project needs a billing account before the API can be activated.

...
The selected implementation preserves Discord's native YouTube preview in one Linky reply or replacement. An attached stats button displays compact counts; the counts and Top comment buttons open private details. The user selected this layout to avoid a separate post. Metadata comes from the official YouTube Data API, is cached for five minutes, and public controls are scheduled for removal after 24 hours. The hosted Linky operator supplies one API key; people adding Linky to a server need no Google credentials. Without a working key, leave YouTube links alone.
...
Linky attaches a legacy action row to the native video message. A live Discord probe confirmed that this preserves the video embed. The journal records the message/video IDs and expiry before count labels are published. Cleanup removes only YouTube controls, preserving the video and unrelated controls. Private button interactions use the shared bounded lookup and require a matching unexpired journal record, bot-owned message, channel and enabled scope. Older companion cards and text suffixes still expire safely. See [single-message layout research](youtube-single-message-layout.md) for the alternatives and constraints.
...
Linky's durable queue stores message/video IDs and expiry times before adding the controls. Cleanup starts after 24 hours, runs at startup and hourly, and retries failed edits. It removes YouTube controls while retaining the native video, message body and unrelated buttons. Existing legacy records still remove their recorded text suffix or companion card. These intervals are implementation choices.

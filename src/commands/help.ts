...
export async function execute(interaction: ChatInputCommandInteraction, settings: Config): Promise<void> {
  const serverEnabled = interaction.guildId !== null && settings.serverIds.includes(interaction.guildId);
  const enabled = settings.channelIds.includes(interaction.channelId) || serverEnabled;
